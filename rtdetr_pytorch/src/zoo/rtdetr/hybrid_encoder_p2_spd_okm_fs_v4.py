...
# 文件说明：
# - 基于v3，两项改动：
#   (1) 去除FreqScale的残差连接（对齐原论文FreDS设计）
#   (2) CCFF融合改为LE-DETR EFAM模式：
#       Concat[128,256,128]=512 → cv1(512→512)重混 → split[128,384]
#       → 创新(128)+identity(384) → cat[512] → cv2(512→512)融合 → CSPRep(512→256)降维
#       不再先降维到256再划分，而是在512维空间做1:3划分后128ch送入创新块
# - 去除FreqScale残差的理由：
#   1. CCFFBlock已有块级残差，FreqScale再加残差造成x_in冗余通路
#   2. FreqScale近零初始化+无残差=渐进生长，符合频域模块从静默到激活的设计哲学
#   3. α/β融合机制已控制FreqScale贡献度，残差绕过了融合控制
#   4. 原论文FreDS无残差连接
# =========================================
'''by lyuwenyu
'''
...
__all__ = ['HybridEncoderP2SPDOKMFSV4']
...
@register
class HybridEncoderP2SPDOKMFSV4(nn.Module):
    def __init__(self,
                 ...
                 fs_num_filters=4,
                 fs_base_size=14,
                 split_ratio=0.25, gmm_patch=4, dcfm_theta=0.875,
                 fs_reweight_ratio=0.25, fs_init_scale=1e-5):
        super().__init__()
        ...
        self.ccff_innovation = CCFFBlock(self.split_channels, gmm_h, gmm_w,
