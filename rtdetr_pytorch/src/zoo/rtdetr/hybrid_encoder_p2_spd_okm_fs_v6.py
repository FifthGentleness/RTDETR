...
# 文件说明：
# - 基于v5，一项改动：
#   (1) PDCBlock去掉stride=1时的内部残差连接（方案二）
#       DCFM输出变为纯差分特征：a·conv_cv(x) + (1-a)·conv_cd(x)
#       Local分支：cat([x, 纯差分]) → small_fuse，x恰好出现1次
#       对齐EFAM语义：创新路径专注差分特征，identity路径负责保底
# - 其余结构同v5：
...
__all__ = ['HybridEncoderP2SPDOKMFSV6']
...
class PDCBlock(nn.Module):
    """v6改动：stride=1时去掉内部残差，输出纯差分特征。

    v5: y = conv2(ReLU(BN(PDCConv(x)))) + x   (含残差)
    v6: y = conv2(ReLU(BN(PDCConv(x))))        (无残差，纯差分)
    stride>1时仍保留shortcut对齐维度。
    """
...
class CCFFBlock(nn.Module):
    """v6: PDCBlock去掉内部残差，DCFM输出纯差分特征。

    Local分支数据流:
        x → DCFM(x) = a·conv_cv(x) + (1-a)·conv_cd(x)   (纯差分，不含x)
        cat([x, DCFM(x)]) → small_fuse                     (x恰好1次)
    """
...
@register
class HybridEncoderP2SPDOKMFSV6(nn.Module):
    def __init__(self,
