# =========================================
# 文件说明：
# - 该文件的作用：实现RT-DETR的改进混合编码器v3，融合MGDFIS的GMM和PiDiViT的DCFM，
#   以及OKNet的OKNetLargeKernel和FDAM的FreqScale，构建三路并行CCFF融合块
#   v3修复v2的关键问题：concat后先Conv1x1降维到hidden_dim再进CCFFBlock，对齐LE-DETR和OKM版本
#   v3数据流：
#   - concat[3C=512] → Conv1x1(512→256) 通道降维融合
#   - → CCFFBlock(256) 内部借鉴LE-DETR的EFAM 1:3划分逻辑：
#     - cv1: Conv1x1(256→256) 通道重混
#     - → split[64, 192] (1:3划分)
#     - → 64ch走创新模块(三路分岔) + 192ch直接透传(identity)
#     - → concat[64, 192]=256 → cv2: Conv1x1(256→256) 融合
#     - → Residual Add
#   创新模块三路分岔设计(split_channels=64)：
#   - Small分支: 原始特征 + DCFM差分增强 → Concat → Conv1x1(128→64)
#   - Large分支: OKNetLargeKernel全向大核空间特征(dw_1×K + dw_K×1 + dw_K×K + dw_1×1)
#   - Global分支: 左分支GMM(空间全局混合) ∥ 右分支FreqScale→FGM(频域调制→频域门控交叉)
#                → 自适应融合 α_c·F_space + β_c·F_cross → Conv1x1+BN+Act
#   三路Element-wise Add → concat[创新输出, identity] → cv2融合 → Residual Add
# =========================================
'''by lyuwenyu
'''
...
__all__ = ['HybridEncoderP2SPDOKMFSV3']
...
class GMM(nn.Module):
    def __init__(self, channels, H, W, patch=4):
        super().__init__()
        assert channels % patch == 0, f'GMM: channels({channels}) must be divisible by patch({patch})'
        assert H == W, f'GMM: requires H==W (got H={H}, W={W}), proj_h/proj_w channel dims depend on H*W symmetry'
...
class FGM(nn.Module):
    """频域门控模块 (Frequency Gate Module) - 来自OKNet原版
    dwconv1(x) 门控 fft2(dwconv2(x))，逐空间位置调制频域
    初始 alpha=0, beta=1 → 恒等映射，渐进介入
    """
...
class CCFFBlock(nn.Module):
    def __init__(self, channels, gmm_h, gmm_w, large_kernel=31,
                 fs_group=16, fs_num_filters=4, fs_base_size=14,
                 split_ratio=1.0, gmm_patch=4, dcfm_theta=0.875,
        ...
        self.gmm = GMM(sc, gmm_h, gmm_w, patch=gmm_patch)
    ...
    def forward(self, x):
        ...
        f_cross = self.fgm(f_freq)
        global_fused = self.alpha * f_space + self.beta * f_cross
...
@register
class HybridEncoderP2SPDOKMFSV3(nn.Module):
    def __init__(self,
        ...
        # CCFF v3: concat[3C] → Conv1x1(3C→C)降维 → CCFFBlock(C, split) → 输出C
        # 对齐LE-DETR: concat[512] → Conv1x1(512→256) → CCFFBlock(256): split[64,192] → 创新(64)+identity(192) → concat → cv2融合 → residual
        ...
        self.ccff_block = CCFFBlock(hidden_dim, gmm_h, gmm_w, large_kernel=large_kernel,
                                    fs_group=fs_group,
                                    fs_num_filters=fs_num_filters, fs_base_size=fs_base_size,
                                    split_ratio=split_ratio, gmm_patch=gmm_patch,
    ...
    def forward(self, feats):
        ...
        # CCFF v3: concat[3C] → Conv1x1(3C→C)降维 → CCFFBlock(C) → 输出C
