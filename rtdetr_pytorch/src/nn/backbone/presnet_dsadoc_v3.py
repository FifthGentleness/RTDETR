...
__all__ = ['DSADOC_v3', 'DSADOCv3BasicBlock', 'PResNet_DSADOC_v3']
...
class DSADOC_v3(nn.Module):
    """Dual Spatial Aggregation & Direction-Omnidirectional Channel calibration v3.

    Same module architecture as DSADOC_v2 (with cross_gate fusion).
    The difference is in PResNet_DSADOC_v3: DSADOC_v3 replaces the last
    convolution block in ALL 4 stages (stage 1-4), whereas v2 only
    replaces stages 2-3.
...
@register
class PResNet_DSADOC_v3(PResNet):
    def __init__(self, depth, variant='d', num_stages=4, return_idx=[0, 1, 2, 3],
        ...
        assert depth in [18, 34], \
            f"PResNet_DSADOC_v3 only supports BasicBlock-based models (depth 18/34), got {depth}"
        ...
        if pretrained:
            if pretrained_source == 'torchvision':
                ...
                print(f'Load PResNet_DSADOC_v3{depth} state_dict from PaddlePaddle')
                if missing:
                    print(f'  DSADOC_v3 params randomly initialized: {len(missing)} keys')
