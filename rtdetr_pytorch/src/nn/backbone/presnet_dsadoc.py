...
__all__ = ['DSADOC', 'DSADOCBasicBlock', 'PResNet_DSADOC']
...
class DSADOC(nn.Module):
    """Dual Spatial Aggregation & Direction-Omnidirectional Channel calibration.

    Improvements over DSADCC_v4:
    1. Direct fusion: removed cross_gate to preserve branch differentiation.
       - v4's cross_gate (Sigmoid ∈ (0,1)) smoothed both branches' signals,
         compressing inter-branch differences and limiting complementary gains.
       - Direct concat + fusion allows the 1x1 conv to learn channel weights
         without signal suppression, enabling stronger DSA/DCC differentiation.
    2. Omnidirectional LL calibration: LL subband now has its own spatial
       feature generated from ALL 4 subbands (LL+LH+HL+HH), replacing the
       mismatched spatial_lh * 0.1 hack.
       - LL is directionless (low-freq structure), so its spatial feature
         should also be directionless (omnidirectional).
       - Using all 4 subbands as input lets the network learn how to combine
         low-freq structure + high-freq edges into an omnidirectional map.
       - Each subband now has semantically matched spatial calibration:
         LL → omnidirectional, LH → horizontal, HL → vertical, HH → diagonal.
    ...
    Architecture:
      ...
      Fusion:     Direct concat [fdsa, fdcc] → Conv1x1+BN+SiLU.
    """
    ...
    def __init__(self, dim, reduction=4):
        ...
        # --- Direct fusion: Concat -> Conv1x1+BN+SiLU ---
...
class PResNet_DSADOC(PResNet):
    def __init__(self, depth, variant='d', num_stages=4, return_idx=[0, 1, 2, 3],
        ...
        assert depth in [18, 34], \
            f"PResNet_DSADOC only supports BasicBlock-based models (depth 18/34), got {depth}"
        ...
        if pretrained:
            if pretrained_source == 'torchvision':
                ...
                print(f'Load PResNet_DSADOC{depth} state_dict from PaddlePaddle')
                if missing:
                    print(f'  DSADOC params randomly initialized: {len(missing)} keys')
