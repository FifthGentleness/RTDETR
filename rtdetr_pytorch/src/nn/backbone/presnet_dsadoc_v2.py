...
__all__ = ['DSADOC_v2', 'DSADOCv2BasicBlock', 'PResNet_DSADOC_v2']
...
class DSADOC_v2(nn.Module):
    """Dual Spatial Aggregation & Direction-Omnidirectional Channel calibration v2.

    Combines the best of DSADCC_v4 and DSADOC:
    1. From DSADCC_v4: Lightweight cross-interaction (cross_gate) at fusion.
       - cross_gate enables DSA and DCC branches to exchange channel-level
         signals before final fusion, improving complementary learning.
    2. From DSADOC: Omnidirectional LL subband calibration.
       - LL subband uses spatial feature from ALL 4 subbands (LL+LH+HL+HH),
         replacing the mismatched spatial_lh * 0.1 hack in DSADCC_v4.
       - Each subband has semantically matched spatial calibration:
    ...
    def forward(self, x):
        ...
        # Omnidirectional spatial feature for LL (all 4 subbands) - from DSADOC
...
@register
class PResNet_DSADOC_v2(PResNet):
    def __init__(self, depth, variant='d', num_stages=4, return_idx=[0, 1, 2, 3],
        ...
        assert depth in [18, 34], \
            f"PResNet_DSADOC_v2 only supports BasicBlock-based models (depth 18/34), got {depth}"
        ...
        if pretrained:
            if pretrained_source == 'torchvision':
                ...
                print(f'Load PResNet_DSADOC_v2{depth} state_dict from PaddlePaddle')
                if missing:
                    print(f'  DSADOC_v2 params randomly initialized: {len(missing)} keys')
