...
__all__ = ['DSADCC_v4', 'DSADCCv4BasicBlock', 'PResNet_DSADCC_v4']
...
class DSADCC_v4(nn.Module):
    """Dual Spatial Aggregation & Dynamic Channel Calibration v4
...
@register
class PResNet_DSADCC_v4(PResNet):
    def __init__(self, depth, variant='d', num_stages=4, return_idx=[0, 1, 2, 3],
        ...
        assert depth in [18, 34], \
            f"PResNet_DSADCC_v4 only supports BasicBlock-based models (depth 18/34), got {depth}"
        ...
        if pretrained:
            if pretrained_source == 'torchvision':
                ...
                print(f'Load PResNet_DSADCC_v4{depth} state_dict from PaddlePaddle')
                if missing:
                    print(f'  DSADCC_v4 params randomly initialized: {len(missing)} keys')
